...
def from_custom_file(data_file: str) -> tuple:
    ...
    for col in mandatory_columns:
        assert col in data.columns, f"inputs for prediction must include these columns: {mandatory_columns}"
...
def from_file(data_file: str,
              data_file2: str,
              learn_options: dict) -> tuple:
    if learn_options["V"] == 1:  # from Nature Biotech paper
        ...
        annotations, gene_position, target_genes, x_df, y_df = read_V1_data(data_file, learn_options)
        ...
        # gene_position, selected_ind, target_genes, x_df, y_df= extract_by_organism("mouse", x_df,y_df, gene_position)

    elif learn_options["V"] == 2:  # from Nov 2014, hot off the machines
        x_df, drugs_to_genes, target_genes, y_df, gene_position = read_V2_data(data_file, learn_options)
        ...
        x_df, y_df, gene_position, target_genes = mergeV1_V2(data_file, data_file2, learn_options)
        ...
        x_df, y_df, gene_position, target_genes = merge_all(data_file, data_file2, learn_options)
...
def combine_organisms(human_data: pd.DataFrame,
                      mouse_data: pd.DataFrame) -> tuple:
    ...
    for k in mouse_data.index.levels[1]:
        # is k the gene
        x_df, y_df = util.get_data(mouse_data.xs(k,
                                                 level='Target',
                                                 drop_level=False),
                                   ["On-target Gene"],
                                   target_gene=k,
...
def read_V1_data(data_file: str = None,
                 learn_options: dict = None,
                 aml_file: str = f"{CUR_DIR}/data/V1_suppl_data.txt") -> tuple:
    ...
    gene_position = pd.merge(x_df, annotations, how="inner", left_index=True, right_index=True)
    ...
    y_df['test'] = 1  # for bookkeeping to keep consistent with V2 which uses this for "extra pairs"

    target_genes = y_df['Target gene'].unique()

    y_df.index.names = ['Sequence', 'Target gene']

    assert x_df.index.equals(
       y_df.index), "The index of x_df is different from the index of y_df " \
...
def read_V2_data(data_file: str = None,
                 learn_options: dict = None,
    ...
    # to compare
    # import predict as pr; a1, g1, t1, X1,y_df1 = pr.data_setup()
    # a1.index.names

    data = pd.read_excel(data_file, sheetname="ResultsFiltered", skiprows=range(0, 6 + 1), index_col=[0, 4])
    ...
    if learn_options is not None:
        assert not (learn_options['extra pairs'] and learn_options[
            'all pairs']), "extra pairs and all pairs options (in learn_options) can't be active simultaneously."
    ...
    for drug in drugs_to_genes.keys():
        ...
        for g in genes:
            ...
            if verbose:
                print(f"Loaded {xtmp.shape[0]} samples for gene {g} \ttotal number of samples: {count}")
    ...
    target = pd.DataFrame(x_df.index.get_level_values('Target gene').values, index=y_df.index, columns=["Target gene"])
    ...
    for drug in drugs_to_genes.keys():
        ...
        for gene in gene_list:
            ytmp = pd.DataFrame(y_df.xs((gene, drug), level=["Target gene", "drug"], drop_level=False)['score'])
            y_ranktmp, y_rank_raw, y_thresholdtmp, y_quanttmp = util.get_ranks(ytmp,
                                                                               thresh=0.8,
                                                                               prefix="score_drug_gene",
                                                                               flip=False)
    ...
    y_quant = pd.DataFrame()
    for drug in drugs_to_genes.keys():
        ytmp = pd.DataFrame(y_df.xs(drug,
                                    level="drug",
    ...
    if learn_options is not None and learn_options["weighted"] == "variance":
        ...
        variance = None
        for drug in drugs_to_genes.keys():
            data_tmp = data.iloc[data.index.get_level_values('Target gene').isin(drugs_to_genes[drug])][
                experiments[drug]]
            data_tmp["drug"] = drug
        ...
        y_df = pd.merge(y_df, pd.DataFrame(variance), how="inner", left_index=True, right_index=True)
...
def mergeV1_V2(data_file: str,
    ...
    annotations, gene_position1, target_genes1, x_df1, y_df1 = read_V1_data(data_file, learn_options)
    x_df2, drugs_to_genes, target_genes2, y_df2, gene_position2 = read_V2_data(data_file2)

    y_df1.rename(columns={'average rank': learn_options["rank-transformed target name"]}, inplace=True)
    y_df1.rename(columns={'average threshold': learn_options["binary target name"]}, inplace=True)
    ...
    if save_to_file:
        ...
        alldupind = np.where(y_df.index.get_level_values(0).values == y_df.index[onedupind][0])[0]
        ...
        newindex[onedupind] = (newindex[onedupind][0],
        ...
        gene_position_tmp.index = pd.MultiIndex.from_tuples(newindex, names=y_df.index.names)
        x_and_y = pd.merge(x_and_y, gene_position_tmp, how="inner", left_index=True, right_index=True)
...
def get_V1_genes(data_file=None,
                 learn_options: dict = None) -> np.ndarray:
    annotations, gene_position, target_genes, x_df, y_df = read_V1_data(data_file, learn_options)
    return target_genes


def get_V2_genes(data_file: str = None) -> np.ndarray:
    x_df, drugs_to_genes, target_genes, y_df, gene_position = read_V2_data(data_file, verbose=False)
    return target_genes


def get_V3_genes(data_fileV1: str = None,
                 data_fileV2: str = None) -> np.ndarray:
...
def get_mouse_genes(data_file: str = None) -> np.ndarray:
    annotations, gene_position, target_genes, x_df, y_df = read_V1_data(data_file, learn_options=None)
