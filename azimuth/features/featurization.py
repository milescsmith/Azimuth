...
def featurize_data(data, learn_options, Y, gene_position, pam_audit=True, length_audit=True, quiet=True):
    ...
    if learn_options["include_gene_effect"]:
        ...
        feature_sets["gene effect"] = DataFrame(one_hot_genes,
    ...
    if learn_options["include_drug"]:
        ...
        feature_sets["drug"] = DataFrame(one_hot_drugs,
...
def get_all_order_nuc_features(data, feature_sets, learn_options, maxorder, max_index_to_use, prefix="", quiet=False):
    for order in range(1, maxorder + 1):
        ...
        nuc_features_pd, nuc_features_pi = apply_nucleotide_features(data, order,
...
def local_gene_seq_features(gene_names, learn_options, X):
    ...
    for gene in gene_names.unique():
        ...
        for ps in np.where(gene_names.values == gene)[0]:
            ...
            assert not left_win.tostring() == "", f"k_mer_context, {k_mer_length}, is too large"
            assert not left_win.tostring() == "", f"k_mer_context, {k_mer_length}, is too large"
            assert len(left_win) == len(right_win), f"k_mer_context, {k_mer_length}, is too large"
    ...
    feature_sets = {}
    get_all_order_nuc_features(feat["gene_left_win"], feature_sets, learn_options, learn_options["order"],
                               max_index_to_use=sys.maxsize, prefix="gene_left_win")
    get_all_order_nuc_features(feat["gene_right_win"], feature_sets, learn_options, learn_options["order"],
                               max_index_to_use=sys.maxsize, prefix="gene_right_win")
    return feature_sets
...
def gene_feature(Y):
    ...
    df = DataFrame(data=all, index=gene_names.index, columns=['gene length',
                                                                     'gene GC content',
                                                                     'gene temperature',
...
def Tm_feature(data, pam_audit=True, learn_options=None):
    ...
    feat = DataFrame(featarray, index=data.index,
                            columns=[f"Tm global_{rna}", f"5mer_end_{rna}", f"8mer_middle_{rna}",
                                     f"5mer_start_{rna}"])
...
def apply_nucleotide_features(seq_data_frame, order, include_pos_independent, max_index_to_use, prefix=""):
    ...
    if feature_type == 'all' or feature_type == 'pos_independent':
        if feature_type == 'all':
            res = Series(features_pos_dependent, index=index_dependent).append(
                Series(features_pos_independent,
...
def nucleotide_features_dictionary(prefix=''):
    ...
    for order in orders:
        ...
        index_dependent.extend([f'{prefix}_pd.Order{order}_P{i}' for i in range(len(features_pos_dependent))])
        index_independent.extend(
            [f'{prefix}_pi.Order{order}_P{i}' for i in range(len(features_pos_independent))])
