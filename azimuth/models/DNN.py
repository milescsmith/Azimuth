    ...
    for i, hl in enumerate(num_hidden_layers):
        for j, nu in enumerate(num_units):
            ...
            for train_ind, valid_ind in cv:
                ...
                e.train((X_train[train_ind], y_train[train_ind]), (X_train[valid_ind], y_train[valid_ind]))
                pred = e.network.predict(X_train[valid_ind])

                accuracies[i, j] += spearmanr(pred.flatten(), y_train[valid_ind].flatten())[0]
            ...
            print(f"DNN with {hl} hidden layers and {nu} units, accuracy: {accuracies[i, j]:.4f}")
