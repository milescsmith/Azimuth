    ...
    if learn_options['adaboost_version'] == 'python':
        if not learn_options['adaboost_CV']:
            if not classification:
                clf = GradientBoostingRegressor(loss=learn_options['adaboost_loss'],
                ...
                clf = GradientBoostingClassifier(learning_rate=learn_options['adaboost_learning_rate'],
                                                 n_estimators=learn_options['adaboost_n_estimators'],
                                                 subsample=1.0,
                                                 min_samples_split=2,
                                                 min_samples_leaf=1,
            ...
            if learn_options["algorithm_hyperparam_search"] == "bo":
                ...
                print(best)
                clf = GradientBoostingRegressor(n_estimators=learn_options['adaboost_n_estimators'],
                ...
                est = GradientBoostingRegressor(loss=learn_options['adaboost_loss'],
                                                random_state=learn_options['seed'])
                clf = GridSearchCV(est, param_grid,
                                   n_jobs=n_jobs,
                                   verbose=1,
                                   cv=cv,
...
def GBR_stacking(y_train, X_train, X_test):
    ...
    clf = GridSearchCV(est,
                       ...
                       verbose=1,
                       cv=20,
                       scoring=spearman_scoring).fit(X_train, y_train.flatten())

    return clf.predict(X_test)


def GP_stacking(y_train, X_train, X_test):
    m = SparseGPRegression(X_train, y_train, num_inducing=20, kernel=RBF(X_train.shape[1]))
