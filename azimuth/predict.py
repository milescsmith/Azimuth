...
def print_summary(global_metric, results, learn_options, feature_sets, flags):
    ...
    print(f"\t\tmedian {learn_options['metric']} across folds={np.median(results[0]):.{2}}")
...
def get_train_test(test_gene, y_all, train_genes=None):
    ...
    if train_genes is not None:
        ...
        for t_gene in train_genes:
            in_train_genes = np.logical_or(in_train_genes,
...
def cross_validate(y_all, feature_sets, learn_options=None, TEST=False, CV=True):
    ...
    if learn_options["cv"] == "stratified":
        assert not ("extra_pairs" in learn_options or learn_options['extra pairs']), \
    ...
    if num_proc > 1:
        ...
        for i, fold in enumerate(cv):  # i in range(0,len(jobs)):
            ...
            if learn_options["training_metric"] == "AUC":
                extract_fpr_tpr_for_fold(aucs=metrics,
                ...
                extract_spearman_for_fold(metrics=metrics,
